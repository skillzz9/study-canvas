...
function StudyRoomContent() {
  ...
  useEffect(() => {
    ...
    const initializeRoom = async () => {
      ...
      try {
        ...
        if (!isMounted) return;
        setUserData(profile);
        await joinOrCreateGlobalRoom(user.uid, paintingId, totalSessionBlocks || 180, totalMinutes || 60, gridSize, totalLayers);
        await updatePresence(user, profile, paintingId, true);
        onSnapshot(collection(db, "paintings", paintingId, "presence"), (snapshot) => {
          const players = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
          if (isMounted) setCollaborators(players);
        });
      } catch (error) { console.error(error); } finally { if (isMounted) setDataLoading(false); }
    };
    initializeRoom();
    return () => {
      isMounted = false;
  ...
  }, [user, authLoading, paintingId, totalSessionBlocks, totalMinutes]);

  useEffect(() => {
    if (!paintingId) return;
    return onSnapshot(doc(db, "paintings", paintingId), (snapshot) => {
      if (snapshot.exists()) {
  ...
  return (
    <main className="min-h-screen bg-app-bg flex flex-col items-center justify-center transition-colors duration-300">
      <div className="absolute top-6 left-6 z-50 flex gap-4">
        <button onClick={() => router.push("/")} className="p-3 bg-app-card border-4 border-app-border rounded-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,0.3)] transition-all text-app-text">
      ...
      <div className="relative flex flex-col items-center pb-40">
        <div className="w-[400px] h-[400px] relative shadow-2xl bg-white rounded-2xl border-4 border-neutral-800 overflow-hidden">
          ...
          <div className="absolute inset-0 z-10">
            <GridRevealMask revealedCount={revealedCount} fullShuffledIndices={dbShuffledIndices} gridSize={gridSize} currentLayerIndex={currentLayerIndex}>
        ...
        <Stopwatch 
          secondsElapsed={sessionSeconds} totalMinutes={totalMinutes} workerCount={sortedWorkers.length} isSessionComplete={isSessionComplete} onFinish={() => router.push("/")}
          onStart={async () => {
             await updateDoc(doc(db, "paintings", paintingId!), { status: "active", lastStartTime: serverTimestamp(), sessionStartedAt: serverTimestamp(), sessionBaseMs: bankedMs, sessionBaseBlocks: revealedCount });
          }} 
          roomStatus={roomStatus} revealedCount={revealedCount} totalSessionBlocks={totalSessionBlocks}
        />

        {sortedWorkers.map((player, index) => (
          <Avatar 
            key={player.id} 
            isMe={player.id === user?.uid} // Ownership check
            myIndex={index} 
